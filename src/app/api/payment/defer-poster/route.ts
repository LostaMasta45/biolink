...
import { reportTelegramPosterDeferred } from "@/services/telegram-admin-service";
...
export async function POST(request: Request) {
  ...
  const { data: order } = await supabase.from("payment_orders").select("*").eq("order_id", parsed.data.order_id).maybeSingle();
  ...
  await reportTelegramPosterDeferred(order);
