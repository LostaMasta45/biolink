...
export function InboxManager() {
  ...
  const loadSupportData = useCallback(async () => {
    try {
      const [replyResult, templateResult] = await Promise.all([
  ...
  return <div className="space-y-5">
    <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><h2 className="text-2xl font-bold tracking-tight">WhatsApp Inbox</h2><p className="text-sm text-muted-foreground">Chat customer melalui Admin Utama. Ketik <b>/</b> untuk memakai Balas Cepat tanpa memicu automation.</p></div><div className="flex gap-2"><Button variant="outline" onClick={() => void loadConversations(true)}><RefreshCcw />Muat ulang</Button><Button onClick={() => setQuickReplyDialog(true)}><Settings2 />Balas Cepat</Button></div></div>
