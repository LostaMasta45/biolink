...
export function LogsMonitor() {
  ...
  return (
    <div className="space-y-6">
      <PageHeading title="Activity & API Logs" description="Jejak invoice, bot, auto reply, antrean notifikasi, retry, dan delivery dari provider. Pending = diterima provider, sukses = delivered/read." icon={Activity} action={<Button variant="outline" onClick={logs.refresh}><RefreshCw />Refresh</Button>} />
      <Card className="rounded-xl border-border/60">
        <CardContent className="grid gap-3 p-4 sm:grid-cols-2 lg:grid-cols-6">
          ...
          <div className="space-y-2"><Label>Event</Label><Select value={eventType} onValueChange={setEventType}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="all">Semua event</SelectItem><SelectItem value="api.message.send">API Send</SelectItem><SelectItem value="api.message.status">Status Provider</SelectItem><SelectItem value="auto_reply.queued">Auto Reply Queued</SelectItem><SelectItem value="auto_reply.sent">Auto Reply Accepted</SelectItem><SelectItem value="auto_reply.failed">Auto Reply Failed</SelectItem><SelectItem value="notification.queued">Notification Queued</SelectItem><SelectItem value="notification.sent">Notification Accepted</SelectItem><SelectItem value="notification.failed">Notification Failed</SelectItem><SelectItem value="invoice.created">Invoice Created</SelectItem><SelectItem value="invoice.updated">Invoice Updated</SelectItem><SelectItem value="invoice.status_changed">Invoice Status Changed</SelectItem><SelectItem value="payment.paid">Payment Paid</SelectItem><SelectItem value="admin_bot.command.completed">Bot Command Completed</SelectItem><SelectItem value="admin_bot.command.failed">Bot Command Failed</SelectItem></SelectContent></Select></div>
