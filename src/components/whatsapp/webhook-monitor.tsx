...
export function WebhookMonitor() {
  ...
  return (
    <div className="space-y-6">
      ...
      {logs.data.length === 0 ? <EmptyState title="Belum ada webhook log" description="Event incoming, outgoing, payment, delivered, read, dan retry akan tampil di sini." /> : <Card className="overflow-hidden rounded-xl border-border/60"><Table><TableHeader><TableRow><TableHead>Waktu</TableHead><TableHead>Arah</TableHead><TableHead>Event</TableHead><TableHead>Status</TableHead><TableHead>Latency</TableHead><TableHead>Retry</TableHead></TableRow></TableHeader><TableBody>{logs.data.map((log) => <TableRow key={log.id}><TableCell className="whitespace-nowrap">{new Date(log.created_at).toLocaleString("id-ID")}</TableCell><TableCell className="capitalize">{log.direction}</TableCell><TableCell className="font-medium">{log.event_type}</TableCell><TableCell><StatusBadge status={log.status} /></TableCell><TableCell>{log.latency_ms} ms</TableCell><TableCell>{log.retry_count}</TableCell></TableRow>)}</TableBody></Table></Card>}
    </div>
  );
}
